...
def confirm_withdraw_cb(call):
    ...
    w_id = db_lastrowid(
        "INSERT INTO withdrawals (user_id, amount, upi_id, status, created_at, gst_amount, net_amount, method, bonus_taxable_amount) VALUES (?,?,?,?,?,?,?,?,?)",
        (user_id, amount, upi_id, "pending", now, tax["total_tax"], tax["net_amount"], "upi", tax["taxable_bonus_amount"])
    )
    ...
    safe_edit(
        ...
        f"📋 <i>GST/Tax is deducted from your withdrawal amount, not charged extra.</i>\n"
    ...
    try:
        if bool(get_setting("withdraw_qr_enabled")):
            send_withdrawal_qr_to_admins(w_id, user, upi_id, tax["net_amount"], amount, tax["total_tax"])
        ...
        bot.send_message(
            ...
            f"{pe('fly_money')} <b>Withdrawal Amount:</b> ₹{amount:.2f}\n"
            f"{pe('receipt')} <b>GST/Tax Cut:</b> ₹{tax['total_tax']:.2f}\n"
            f"{pe('money')} <b>User Will Receive:</b> ₹{tax['net_amount']:.2f}\n"
            f"{pe('link')} <b>UPI:</b> <code>{upi_id}</code>\n"
...
def admin_approve(call):
    ...
    payout_amt = round(float(wd["net_amount"] or amt), 2)
    gst_amt = round(float(wd["gst_amount"] or 0), 2)
    u = get_user(uid)
    if u:
        # total_withdrawn tracks the requested wallet withdrawal amount, while
        # payout_amt is what the user receives after GST/tax deduction.
        update_user(uid, total_withdrawn=u["total_withdrawn"] + amt)
    log_admin_action(call.from_user.id, "approve_withdrawal", f"Approved WD #{w_id} payout ₹{payout_amt} for {uid}")
    ...
    try:
        safe_send(
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"{pe('fly_money')} <b>Withdrawal Amount:</b> ₹{amt:.2f}\n"
            f"{pe('receipt')} <b>GST/Tax Cut:</b> ₹{gst_amt:.2f}\n"
            f"{pe('money')} <b>Paid Amount:</b> ₹{payout_amt:.2f}\n"
...
def admin_reject(call):
    ...
    bonus_refund = round(float(wd["bonus_taxable_amount"] or 0), 2) if "bonus_taxable_amount" in wd.keys() else 0.0
    u = get_user(uid)
    if u:
        update_user(
            uid,
            balance=u["balance"] + refund_amount,
            bonus_balance=float(u["bonus_balance"] or 0) + bonus_refund,
        )
    ...
    try:
        safe_edit(
            call.message.chat.id, call.message.message_id,
            (call.message.text or "") + f"\n\n{pe('cross')} <b>REJECTED ❌</b> (Withdrawal amount refunded)"
        )
    except Exception:
        pass
    try:
